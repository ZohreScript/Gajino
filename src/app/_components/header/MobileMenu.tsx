        ...
        {openThirdSubMenu !== null ? (
          <div className="flex flex-col min-h-screen">
            <div className="bg-gray-100 border-b border-b-gray-200 p-4 flex items-center justify-end">
              <button
                className="flex items-center justify-end text-base font-semibold text-MokhFont1 cursor-pointer mb-2"
            ...
            {selectedSubSubPage && (
      <div className="text-right p-1 text-MokhFont1">
        همه درس‌های {selectedSubSubPage}
                  ...
                  subPage.subsubPages?.filter((subSubPage) => subSubPage.id === openSubSubMenu)
                    .flatMap((subSubPage) =>
                      subSubPage.thirdsubPages?.map((thirdSubPage) => (
                        ...
                        <li
                          key={thirdSubPage.id}
                          className="text-base border-b p-4 text-MokhFont2 border-gray-100 pb-2"
                        >
                          <Link
                            href={thirdSubPage.url}
                            className={`text-MokhDarkBlue ${currentUrl.startsWith(thirdSubPage.url) ? "font-semibold" : "font-normal"}`}
                            onClick={toggleMenu}
                          >
                            {thirdSubPage.type}
                          </Link>
                ...
                page.subPages?.filter((subPage) => subPage.id === openSubMenu)
                  .flatMap((subPage) =>
                    subPage.subsubPages?.map((subSubPage) => (
                      <li
                        key={subSubPage.id}
                        className={`text-base p-4 border-b text-MokhFont1 border-gray-100 pb-2 ${hoveredSubPage === subSubPage.id ? "bg-gray-200" : ""}`}
                        onMouseEnter={() => setHoveredSubPage(subSubPage.id)}
                        onMouseLeave={() => setHoveredSubPage(null)}
                      >
                        <div
                          className={`flex items-center justify-between rounded-md text-MokhDarkBlue cursor-pointer ${openThirdSubMenu === subSubPage.id ? "font-semibold" : "font-normal"}`}
                        ...
                        >
                          ...
                          {subSubPage.thirdsubPages && (
                            <IoIosArrowBack className="text-MokhFont1" />
                          )}
                        </div>
                      </li>
                    ))
            ...
            <ul className="flex-1 overflow-y-auto">
              {Pages.flatMap((page) =>
                page.subPages?.map((subPage) => (
                  <li
                    key={subPage.id}
                    className={`text-base p-4 border-b text-MokhFont1 border-gray-100 pb-2 ${hoveredSubPage === subPage.id ? "bg-gray-200" : ""}`}
                    onMouseEnter={() => setHoveredSubPage(subPage.id)}
                    onMouseLeave={() => setHoveredSubPage(null)}
                  >
                    <div
                      className={`flex items-right justify-between text-MokhDarkBlue cursor-pointer ${openSubSubMenu === subPage.id ? "font-semibold" : "font-normal"}`}
          ...
          {Pages.map((item: Page) => (
            ...
            >
              <div
                className={`flex items-center justify-between text-MokhBlue cursor-pointer ${
                  currentUrl.startsWith(item.url) ? "font-semibold" : "font-normal"
                }`}
                onClick={() => handleSubMenuClick(item.id)}
              >
                {item.type}
                {item.subPages && <IoIosArrowBack className="text-MokhBlue" />}
              </div>
            </li>
          ))}
